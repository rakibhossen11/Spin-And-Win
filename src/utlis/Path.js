...
const generateBowShape = () => {
  ...
  const points = Array.from({ length: numPoints }, (_, i) => {
    ...
    const x = SCREEN_WIDTH / 2 + radius * Math.cos(angle);
...
const generateWheelSegments = (segments) => {
  ...
  return segments.map((amount, index) => {
    ...
    const pathData = arc({ startAngle, endAngle });
    const centroid = arc.centroid({ startAngle, endAngle });
